...
test "config: parse() reads llm and minimax players" {
    ...
    // No "rules" field in this config: defaults to Spanish.
    try std.testing.expectEqual(config_mod.Variant.spanish, cfg.rules);
}

test "config: rules field selects the variant, defaulting to spanish" {
    const allocator = std.testing.allocator;
    ...
    try std.testing.expectEqual(config_mod.Variant.spanish, missing.rules);

    const invalid = try config_mod.parse(allocator, "{\"rules\":\"frisian\"," ++ players ++ "}");
    try std.testing.expectEqual(config_mod.Variant.spanish, invalid.rules);

    // Non-string "rules" value: ignored, falls back to Spanish, no error.
    const non_string = try config_mod.parse(allocator, "{\"rules\":42," ++ players ++ "}");
    try std.testing.expectEqual(config_mod.Variant.spanish, non_string.rules);

    // parseVariant helper: anything but "english" is Spanish.
    ...
    try std.testing.expectEqual(config_mod.Variant.spanish, config_mod.parseVariant("bogus"));
}
