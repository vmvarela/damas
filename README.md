...
- `damas-z tui` — full-screen terminal UI (uses the `rules` from config.json)
- `damas-z help` — usage

`--rules english|spanish` overrides the variant on any entry point (before or
after the subcommand, e.g. `damas-z --rules spanish` or `damas-z tui --rules
spanish`): it beats `config.json` in the match CLI and TUI, and sets the
server's default variant for the web UI (`damas-z web --rules spanish` starts
